...
export async function updateDeck(
  ...
  if (!res.ok) {
    throw new Error(await errorMessage(res, "failed to update deck"));
  }
}
...
export async function deleteDeck(token: string, id: number): Promise<void> {
  ...
  if (!res.ok) {
    throw new Error(await errorMessage(res, "failed to delete deck"));
  }
}
...
export async function fetchProfile(user: string): Promise<Profile> {
  const res = await fetch(
    serverURL(`/profile?user=${encodeURIComponent(user)}`),
  );
  if (!res.ok) {
    throw new Error(await errorMessage(res, "failed to load profile"));
  }
...
export async function fetchLeaderboard(): Promise<LeaderRow[]> {
  ...
  if (!res.ok) {
    throw new Error(await errorMessage(res, "failed to load leaderboard"));
  }
