...
export function Deckbuilder(props: { token: string; onBack: () => void }) {
  ...
  const [editing, setEditing] = useState<
    { id: number | null; name: string; class: string; cards: string[] } | null
  >(
    null,
  );
  ...
  const [preview, setPreview] = useState<
    { card: CardView; x: number; y: number } | null
  >(null);
  ...
  const activeClass = editing?.class ?? browseClass ?? pool?.classes[0] ??
    "mage";
  ...
  const filtered = useMemo(() => {
    const cs = (pool?.cards ?? []).filter((c) => {
      ...
      if (tab === "all" && editing && cl !== "neutral" && cl !== activeClass) {
        return false;
      }
      if (
        manaFilter !== null &&
        (manaFilter === 7 ? c.cost < 7 : c.cost !== manaFilter)
      ) return false;
  ...
  const copyCode = async (d: Deck) => {
    ...
    try {
      await navigator.clipboard.writeText(
        encodeDeck(d.class, d.cards, pool.cards),
      );
      setCopiedId(d.id);
  ...
  const classLabel = (id: string) =>
    CLASS_OPTIONS.find((o) => o.id === id)?.label ?? id;
  ...
  const deckEditPanel = editing && (
    <div className="deck-edit">
      <div className="edit-head">
        ...
        <span
          className={"meter" +
            (editing.cards.length === deckSize ? " full" : "")}
        >
      ...
      <div className="curve" title="Mana curve">
        {CURVE_SLOTS.map((slot, i) => (
          <div key={slot} className="curve-col">
            ...
            <div
              className="curve-bar"
              style={{ height: `${(curve[i] / curveMax) * 100}%` }}
            />
      ...
      <div className="edit-actions">
        <button
          type="button"
          disabled={editing.cards.length !== deckSize || !editing.name.trim()}
          onClick={save}
        >
          Save
        </button>
      ...
      <div className="in-deck">
        {deckEntries.length === 0 && (
          <p className="empty">Empty — click cards to add.</p>
        )}
          ...
          return (
            <button
              ...
              onMouseEnter={(e) =>
                ...
                setPreview({
                  card,
                  x: (e.currentTarget.closest(".deck-panel") ?? e.currentTarget)
                    .getBoundingClientRect().left,
                  y: e.clientY,
                })}
              onMouseMove={(e) =>
                setPreview((p) => (p ? { ...p, y: e.clientY } : p))}
              onMouseLeave={() => setPreview(null)}
              title="Remove one"
            >
              <span className="cost">{card?.cost}</span>
              <span className={"dc-name" + (r ? ` rarity-${r}` : "")}>
                {card?.name}
              </span>
      ...
      <div className="builder-inner">
        ...
        <div className="builder">
              ...
              <div className="book-tabs">
                ...
                {(editing ? [editing.class] : pool.classes).map((cls) => (
                  <button
                    type="button"
                    key={cls}
                    className={"tab-tile tab-portrait" +
                      (tab === "class" && activeClass === cls ? " on" : "")}
                ...
                >
                  ...
                  >
                    <path
                      d="M12 2.5 20 7 V17 L12 21.5 4 17 V7 Z"
                ...
                <span className="rail-meta">
                  ...
                  >
                    ...
                    {(manaFilter !== null || rarityFilter !== null ||
                      tribeFilter !== null) && (
              ...
              <div className="book">
                ...
                <div className="book-grid">
                  {pageCards.map((c) => {
                    ...
                    const maxed = !editing || n >= capFor(c) ||
                      editing.cards.length >= deckSize;
                    return (
                      <button
                        type="button"
                 ...
                 zero cards match, where these hold the columns up under the message. */
                  ...
                  {Array.from(
                    { length: perPage - pageCards.length },
                    (_, i) => (
                      <div
                        key={`pad-${i}`}
          ...
          <aside className="deck-panel">
            ...
            {!editing && (
              <p className="hint">Pick a deck to edit, or create a new one.</p>
            )}
          </aside>
        </div>
      </div>
                    ...
                    >
                      {enabled && (
                        <span
                          ...
                          style={{
                            backgroundImage: `url('/art/${o.id}_hero.png')`,
                          }}
                        />
                      )}
                      <span className="class-name">{o.label}</span>
                      {!enabled && (
                        <span className="class-soon">Coming soon</span>
                      )}
                    </button>
                  );
                })}
