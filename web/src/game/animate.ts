...
export function animDuration(events?: Event[]): number {
  ...
  let diff = 0;
  if (summons) {
    diff = Math.max(diff, 650 + Math.max(0, summons - 1) * 320 + 700); // staggered token pops
  }
...
const el = (cid: string) =>
  document.querySelector<HTMLElement>(`[data-cid="${CSS.escape(cid)}"]`);
...
export function hitFlash(targetCid: string, amount?: number) {
  ...
  t.animate(
    [
      ...
      {
        filter: "brightness(2.2) drop-shadow(0 0 8px #f55)",
...
export function projectile(
  ...
  const anim = node.animate(
    [
      ...
      {
        transform:
          `translate(calc(-50% + ${dx}px), calc(-50% + ${dy}px)) scale(1.1)`,
        opacity: 1,
        offset: 0.8,
      },
      {
        transform:
          `translate(calc(-50% + ${dx}px), calc(-50% + ${dy}px)) scale(1.8)`,
        opacity: 0,
      },
    ],
...
export function fatigueBurst(side: "self" | "opp") {
  ...
  node.innerHTML =
    '<span class="fatigue-skull">💀</span><span class="fatigue-name">Fatigue</span>';
  Object.assign(node.style, {
    position: "fixed",
    left: `${r.left}px`,
    top: `${r.top}px`,
  ...
  const anim = node.animate(
    [
      ...
      {
        transform: `translate(${cx}px, ${cy}px) scale(2.2) rotate(0)`,
        opacity: 1,
        offset: 0.45,
      },
      {
      ...
      {
        transform: `translate(${cx}px, ${cy - 24}px) scale(2.4) rotate(6deg)`,
        opacity: 0,
      },
    ],
...
export function summonPop(minionCid: string, duration = 700, delay = 0) {
  ...
  const anim = m.animate(
    ...
    ],
    {
      ...
      delay,
      fill: "backwards",
      easing: "cubic-bezier(.2,.85,.3,1.1)",
    },
  );
...
function slideFrom(
  ...
  const anim = node.animate(
    [
      {
        transform: `translate(${dx}px, ${dy}px) scale(.62) rotate(-5deg)`,
        opacity: 0,
        offset: 0,
      },
...
function flyToCenter(node: HTMLElement, from: DOMRect) {
  ...
  const anim = node.animate(
    [
      ...
      {
        transform: `translate(${cx}px, ${cy}px) scale(1.45)`,
        opacity: 1,
        offset: 0.6,
      },
