...
function CardArt({ card }: { card: CardView }) {
  ...
  if (failed) {
    return <span className="card-art-glyph">{cardArtIcon(card.cardType)}</span>;
  }
...
export function CardFace({ card }: { card: CardView }) {
  ...
  const right = card.cardType === "weapon"
    ? (card.durability ?? 0)
    : card.health;
  ...
  const costCls = card.baseCost != null && card.cost !== card.baseCost
    ? card.cost < card.baseCost ? " cost-reduced" : " cost-raised"
    : "";
  ...
  const nameStyle = longestWord >= 13 || nameLen >= 20
    ? { fontSize: "8px" }
    : longestWord >= 11 || nameLen >= 14
    ? { fontSize: "9px" }
    : undefined;
  ...
  return (
    <>
      ...
      <div className="card-art">
        ...
        <div
          className={"name" + (card.rarity ? ` rarity-${card.rarity}` : "")}
          style={nameStyle}
        >
          {card.name}
        </div>
