...
export function formatEvent(
  ...
  switch (e.kind) {
    ...
    case "burn":
      return `${ent(e.target)} burns ${
        e.card ? e.card.name : "a card"
      } (hand full)`;
    case "generate":
...
function actorFor(
  ...
  if (id[0] !== "u") {
    return {
      name: id === me ? "Your hero" : "Enemy hero",
      hero: id === me ? "self" : "opp",
    };
  }
  const card = cards[id];
  return {
    name: card?.name ?? names[id] ?? id,
    uid: id,
    cardId: card?.cardId,
    card,
  };
...
export function buildLog(
  ...
  const cur = (e: Event): LogEntry =>
    group ??
      open({
        kind: e.kind,
        text: formatEvent(e, names, me),
        source: A(e.source),
        targets: [],
      });
    ...
    switch (e.kind) {
      ...
      case "heropower": {
        const g = open({
          ...
          text,
          source: A(e.source),
          note: e.kind === "heropower" ? e.name : undefined,
          targets: [],
        });
        ...
        events.forEach((d, j) => {
          ...
          g.targets.push(t);
          if (d.target && (d.kind === "damage" || d.kind === "shield")) {
            lethalTarget[d.target] = t;
          }
        });
        return;
      }
      ...
      case "buff": {
        ...
        const dh = e.buffHp ?? 0;
        const before: LogActor | undefined = after?.card && (da || dh)
          ? {
            ...after,
            card: {
              ...after.card,
              attack: after.card.attack - da,
              health: after.card.health - dh,
            },
          }
          : undefined;
        add(e, {
      ...
      case "burn":
        ...
        open({
          ...
          targets: e.card
            ? [{
              actor: { name: e.card.name, cardId: e.card.cardId, card: e.card },
              kind: e.kind,
            }]
            : [],
        });
      ...
      default:
        ...
        open({
          ...
          text,
          source: undefined,
          targets: [{ actor: A(e.target), kind: e.kind, amount: e.amount }],
        });
    }
  });
  return rows;
