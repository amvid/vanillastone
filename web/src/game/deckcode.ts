...
//   classChar : m=mage h=hunter r=warrior w=warlock p=priest l=paladin d=druid g=rogue s=shaman
...
function buildMaps(
  pool: CardView[],
): { idToToken: Record<string, string>; tokenToId: Record<string, string> } {
...
export function encodeDeck(
  ...
  const entries = Object.entries(counts).map(([id, n]) => {
    ...
    if (n < 1 || n > 9) {
      throw new Error(`unsupported copy count ${n} for ${id}`);
    }
    return token + n;
  });
...
export function decodeDeck(
  ...
  if (checksum(cls, cards) !== sum) {
    throw new Error("deck code is invalid or from an older card set");
  }
