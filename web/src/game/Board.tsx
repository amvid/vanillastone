...
export function Board(props: {
  ...
  const nodes = minions.map((m) => {
    ...
    if (m.stealth) {
      badges.push({
        icon: "🌫️",
        label: "Stealth",
        desc: "Can't be targeted by the enemy until it attacks.",
      });
    }
    if (m.poisonous) {
      badges.push({
        icon: "🧪",
        label: "Poisonous",
        desc: "Any minion it damages is destroyed.",
      });
    }
    if (m.lifesteal) {
      badges.push({
        icon: "🩸",
        label: "Lifesteal",
        desc: "Damage it deals also heals your hero.",
      });
    }
    if (m.finalGasp) {
      badges.push({
        icon: "💀",
        label: "Final Gasp",
        desc: "Triggers an effect when this minion dies.",
      });
    }
    if (m.spellDamage) {
      badges.push({
    ...
    if (m.cantAttack) {
      badges.push({
        icon: "🚫",
        label: "Can't Attack",
        desc: "This minion cannot attack.",
      });
    }
    return (
      <div
      ...
      >
        ...
        {m.frozen && (
          <div
            className="frost-badge"
            title="Frozen — can't attack this turn"
            aria-hidden="true"
          >
            ❄
          </div>
        ...
        {m.silenced && (
          <div
            className="silence-mark"
            title="Silenced — has lost all abilities"
            aria-hidden="true"
          >
            🔇
          </div>
        ...
        <div className={"m-nametag" + (m.rarity ? ` rarity-${m.rarity}` : "")}>
          {m.name}
        </div>
        {badges.length > 0 && (
        ...
        <div className={"m-atk" + statClass(m.attack, m.baseAttack)}>
          {m.attack}
        </div>
        <div
          className={"m-hp" + (hp < m.maxHealth
            ? " stat-down"
            : statClass(m.maxHealth, m.baseHealth))}
        >
          {hp}
        </div>
  ...
  if (pending) {
    const at = pending.pos == null
      ? nodes.length
      : Math.max(0, Math.min(pending.pos, nodes.length));
