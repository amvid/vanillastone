...
export function GameScreen(props: GameScreenProps) {
  ...
  const pendingCard = spell
    ? (snap.self.hand?.[spell.handIndex] ?? null)
    : null;
  const pendingMinion = pendingCard && pendingCard.cardType === "minion"
    ? { card: pendingCard, pos: spell?.pos ?? null }
    : null;
  ...
  const sourceId = attacker
    ? attacker
    : spell
    ? pendingMinion ? "pending" : `hand-${spell.handIndex}`
    : heroPowerArmed
    ? "heroPower"
    : null;
  ...
  const [logPop, setLogPop] = useState<
    { entry: LogEntry; rightPx: number; topPx: number } | null
  >(null);
  ...
  const [placing, setPlacing] = useState<
    { handIndex: number; card: CardView } | null
  >(null);
  ...
  const pressRef = useRef<
    { handIndex: number; card: CardView; x: number; y: number } | null
  >(null);
  ...
  >(null);
  const castTimer = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined,
  );
  ...
  const [secretFx, setSecretFx] = useState<
    { key: number; name: string } | null
  >(null);
  const secretFxTimer = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined,
  );
  ...
  const fireSecretFx = useCallback((name: string) => {
    ...
    secretFxTimer.current = globalThis.setTimeout(
      () => setSecretFx(null),
      3500,
    );
  }, []);
  ...
  >(null);
  const burnFaceTimer = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined,
  );
  ...
  const showBurnFace = useCallback((card: CardView, side: "self" | "opp") => {
    ...
    burnFaceTimer.current = globalThis.setTimeout(
      () => setBurnFace(null),
      2000,
    );
  }, []);
  ...
  const charUnder = useCallback((x: number, y: number): string | null => {
    ...
    if (
      s.opp.board.some((mn) => mn.instanceId === cid) ||
      s.self.board.some((mn) => mn.instanceId === cid)
    ) return cid;
    return null;
  }, []);
  ...
  useEffect(() => {
    const onOver = (e: MouseEvent) => {
      ...
      const node = (e.target as HTMLElement)?.closest?.("[data-cid]") as
        | HTMLElement
        | null;
      const cid = node?.getAttribute("data-cid") ?? "";
      const s = snapRef.current;
      const isMinion = cid[0] === "u" &&
        (s.opp.board.some((m) => m.instanceId === cid) ||
          s.self.board.some((m) => m.instanceId === cid));
  ...
  useLayoutEffect(() => {
    ...
    for (const e of anim.events) {
      if (e.kind === "damage" && e.target) {
        ...
        if (
          c && old[c] !== undefined && newHp[c] !== undefined &&
          newHp[c] < old[c]
        ) holds[c] = old[c];
      }
    }
    ...
    setHeldHp((
      h,
    ) => (Object.keys(holds).length || Object.keys(h).length ? holds : h));
  ...
  useLayoutEffect(() => {
    ...
    const from = introFrom ??
      { x: globalThis.innerWidth / 2, y: globalThis.innerHeight * 0.42 };
    for (let i = 0; i < openCount; i++) {
      dealIn(`hand-${i}`, from, i * STAGGER, 700);
    }
    if (drawLast) {
      dealIn(
        `hand-${n - 1}`,
        ".deck-pile.self",
        openCount * STAGGER + 420,
        850,
      );
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useLayoutEffect(() => {
    ...
    if (prev) {
      ...
      for (const e of ev) {
        if (e.kind === "summon" && e.target) summonIds.add(e.target);
      }
      ...
      const newMinions = [...snap.self.board, ...snap.opp.board].filter((mn) =>
        !prev.has(mn.instanceId)
      );
      const hasPlayedMinion = newMinions.some(
        (mn) =>
          playedName && mn.name === playedName && summonIds.has(mn.instanceId),
      );
      let flewPlayed = false;
      let tokenIdx = 0;
      const animateNew = (mn: MinionView, from: string) => {
        if (
          !flewPlayed && playedName && mn.name === playedName &&
          summonIds.has(mn.instanceId)
        ) {
          flewPlayed = true;
      ...
      for (const mn of snap.self.board) {
        if (!prev.has(mn.instanceId)) animateNew(mn, ".hand");
      }
      for (const mn of snap.opp.board) {
        if (!prev.has(mn.instanceId)) animateNew(mn, ".opp-hand");
      }
    ...
    if (!intro) {
      ...
      for (let k = 0; k < selfDraws; k++) {
        flyIn(`hand-${sh - 1 - k}`, ".deck-pile.self", 1100);
      }
  ...
  useEffect(() => {
    ...
    const dstOf = (cid: string): string | DOMRect | null =>
      document.querySelector(`[data-cid="${CSS.escape(cid)}"]`)
        ? cid
        : (rectCache.current[cid] ?? null);
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      for (let j = i + 1; j < events.length; j++) {
        if (events[j].kind === "damage" || events[j].kind === "shield") {
          causeOf[j] = i;
        } else break;
      }
    }
    ...
    const oppPlay = [...events].reverse().find((e) =>
      e.kind === "play" && e.source !== snap.you && e.card
    );
    ...
    const at = (ms: number, fn: () => void) =>
      void timers.push(globalThis.setTimeout(fn, ms));
    ...
    if (startGames.length) {
      ...
      startGames
        .sort((a, b) =>
          (a.source === snap.you ? 0 : 1) - (b.source === snap.you ? 0 : 1)
        )
        .forEach((e, k) => {
          ...
          at(
            ...
            () =>
              showReveal({
                card: e.card,
                label: `🔮 ${mine ? "Your" : "Opponent's"} Start of Game`,
              }),
          );
        });
    }
    ...
    events.forEach((e, i) => {
      ...
      if (e.kind === "attack") {
        ...
        const counteredCard = e.note
          ? events.find((p) =>
            p.kind === "play" && p.card?.cardType === "spell" &&
            p.card?.name === e.note
          )?.card
          : undefined;
  ...
  useEffect(() => {
    ...
    const captureCenters = () => {
      ...
      document.querySelectorAll<HTMLElement>(".zone.bottom .board [data-cid]")
        .forEach((n) => {
    ...
    const overTable = (x: number, y: number): boolean => {
      ...
      if (hand && y >= hand.top && x >= hand.left && x <= hand.right) {
        return false;
      }
      const area = document.querySelector(".play-area")
        ?.getBoundingClientRect();
      return !!area && x >= area.left && x <= area.right && y >= area.top &&
        y <= area.bottom;
    };
    ...
    const commit = (d: NonNullable<typeof drag>, x: number, y: number) => {
      ...
      onHandCard(
        d.handIndex,
        d.card,
        d.card.cardType === "minion" ? slotFor(x) : undefined,
      );
    };
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (d) {
        setDrag({
          ...
          x: e.clientX,
          y: e.clientY,
          pos: posFor(d.card, e.clientX, e.clientY),
        });
  ...
  return (
    ...
    >
      <div className="game-stage">
        <div
          className={"play-area" +
            (!spectator && myTurn && !winner && lowTime ? " low-time" : "")}
        >
            ...
            <div className="overlay gameover-overlay">
              ...
              >
                <div className="go-result">
                  {spectator
                    ? `${
                      (winner === "you" ? snap.self.name : snap.opp.name) ||
                      "Player"
                    } wins`
                    : winner === "you"
                    ? "Victory!"
                ...
                {!spectator && (
                  <div className="go-sub">vs {snap.opp.name || "Opponent"}</div>
                )}
                {
                    ...
                    return (
                      <div className={"go-rank " + cls}>
                        ...
                        <span className="go-rank-delta">
                          {arrow}{" "}
                          {oldRank === 0 ? "New rank" : `from #${oldRank}`}
                        </span>
                      </div>
                    );
                  })()
                ...
                ? (
                  <>
                    ...
                    <div className="cast-text">
                      No cards left to draw — takes increasing damage each turn.
                    </div>
                  </>
                )
                : cast.secret
                  ...
                  cast.card && (
                    <div className="cast-pair">
                      {cast.counteredCard && (
                        <>
                          <div
                            className={"card cast-card countered" +
                              cardColorClass(cast.counteredCard)}
                          >
                      ...
                      <div
                        className={"card cast-card" + cardColorClass(cast.card)}
                      >
          ...
          {burnFace && (
            ...
            >
              <div
                className={"card burn-show-card" +
                  cardColorClass(burnFace.card)}
              >
          ...
          <DeckPile
            side="opp"
            count={snap.opp.deckCount}
            mana={snap.opp.mana}
            max={snap.opp.maxMana}
          />
          <DeckPile
            side="self"
            count={snap.self.deckCount}
            mana={snap.self.mana}
          ...
          <div className="player-name opp">
            {snap.opp.rank
              ? <span className="pn-rank">#{snap.opp.rank}</span>
              : null}
            {snap.opp.name || "Opponent"}
          </div>
          <div className="player-name self">
            {snap.self.rank
              ? <span className="pn-rank">#{snap.self.rank}</span>
              : null}
            {snap.self.name || name}
          </div>
          ...
          <div className="zone top">
            ...
            <div className="self-row">
              {snap.opp.heroPower && (
                <div
                  className={"hp-button static" +
                    (snap.opp.heroPowerUsed ? " used" : "")}
                  data-cid="oppHeroPower"
                  ...
                  <span className="hp-flip">
                    <span className="hp-face front">
                      <span
                        ...
                        style={{
                          backgroundImage:
                            `url(/art/${snap.opp.heroPower.cardId}.png)`,
                        }}
                      >
            ...
            <div className="turn-chip">
              <span className={"turn-flag" + (myTurn ? " mine" : "")}>
                {winner
                  ...
                  : spectator
                  ? `${
                    (myTurn ? snap.self.name : snap.opp.name) || "Player"
                  }'s turn`
                  : myTurn
                  ? "YOUR TURN"
                  : "Opponent's turn"}
              ...
              {!winner && (
                <TurnTimer key={turnNum} secs={turnSecs} onLow={setLowTime} />
              )}
            </div>
          </div>
          ...
          <div className="zone bottom">
            <Board
              ...
              attacker={attacker}
              dropIndex={drag && drag.card.cardType === "minion"
                ? drag.pos
                : null}
            ...
            <div className="self-row">
              {hp && (
                <button
                  ...
                  className={"hp-button" +
                    ...
                    (!spectator && myTurn && !winner &&
                        !snap.self.heroPowerUsed && hp.cost <= snap.self.mana &&
                        !heroPowerArmed
                      ? " ready"
                      : "")}
                  disabled={spectator || !myTurn || !!winner ||
                    snap.self.heroPowerUsed || hp.cost > snap.self.mana}
                  onClick={onHeroPower}
                >
                  ...
                  <span className="hp-flip">
                    <span className="hp-face front">
                      <span
                        className="hp-art"
                        style={{
                          backgroundImage: `url(/art/${hp.cardId}.png)`,
                        }}
                      >
                        {hpIcon(hp.name)}
                      </span>
              ...
              <Hero
                ...
                ready={!spectator && myTurn && !winner &&
                  !!snap.self.heroCanAttack}
            ...
            {isMobile && handFocused && (
              <div
                className="hand-backdrop"
                onPointerDown={() => setHandFocused(false)}
              />
            )}
            ...
            {isMobile && placing && (
              <div className="placing-hint">
                Tap a slot to place · tap hand to cancel
              </div>
            )}
            ...
            {isMobile && !placing && spell && (
              <div className="placing-hint">
                Tap a target · tap hand to cancel
              </div>
            )}
            ...
            {isMobile &&
              (() => {
                const c = placing
                  ? placing.card
                  : spell
                  ? snap.self.hand?.[spell.handIndex]
                  : null;
                if (!c) return null;
                return (
            ...
            <div
              className={"hand" +
                (isMobile ? (handFocused ? " focused" : " peek") : "")}
              ...
              return (
                <div
                  ...
                  onMouseEnter={(ev) => {
                    ...
                    const top = Math.min(
                      Math.max(r.top + r.height / 2, 8 + H / 2),
                      globalThis.innerHeight - 8 - H / 2,
                    );
                    setLogPop({
                      entry: e,
                      rightPx: globalThis.innerWidth - r.left + 10,
                      topPx: top,
                    });
                  }}
                  onMouseLeave={() => {
                ...
                >
                  ...
                  {e.targets.length > 1 && (
                    <span className="log-count">×{e.targets.length}</span>
                  )}
                </div>
              );
            })}
        ...
        {oppSeek != null && (
          <div className="opp-seek">
            ...
            <div className="opp-seek-cards">
              {Array.from(
                { length: oppSeek },
                (_, i) => <span key={i} className="card-back opp-seek-card" />,
              )}
            </div>
          </div>
        )}
      ...
      {showOppIntent && oppIntent?.aimFrom && oppIntent?.aimTo && (
        <OppAimArrow
          from={oppTokenCenter(oppIntent.aimFrom)}
          to={oppTokenCenter(oppIntent.aimTo)}
        />
      )}
    </div>
  );
...
function LogChip({ actor }: { actor: LogActor }) {
  if (actor.hero) {
    return (
      <span
        className={"log-chip hero " + actor.hero}
        style={{ backgroundImage: "url(/art/mage_hero.png)" }}
      />
    );
  }
  return (
    <span
      className="log-chip"
      style={actor.cardId
        ? { backgroundImage: `url(/art/${actor.cardId}.png)` }
        : undefined}
    />
  );
}
...
function LogActorBig({ actor }: { actor: LogActor }) {
  if (actor.hero) {
    return (
      <div
        className={"log-pop-hero " + actor.hero}
        style={{ backgroundImage: "url(/art/mage_hero.png)" }}
      >
...
function LogDetail(
  ...
  const targets = e.targets.filter(
    (t) =>
      !(t.actor?.uid && t.actor.uid === e.source?.uid && t.kind !== "buff"),
  );
  ...
  const selfOnly = !!e.source?.uid && targets.length === 1 &&
    targets[0].actor?.uid === e.source.uid;
  return (
            ...
            {targets.map((t, i) => (
              <div className="log-pop-target" key={i}>
                ...
                <span className="log-pop-outcome">
                  ...
                  {t.buffAtk || t.buffHp
                    ? (
                      <span className="log-amt buff">
                        +{t.buffAtk ?? 0}/+{t.buffHp ?? 0}
                      </span>
                    )
                    : null}
...
function TurnTimer(
  { secs, onLow }: { secs: number; onLow?: (low: boolean) => void },
) {
...
function DeckPile(
  ...
  return (
    <div className={"deck-pile " + side}>
      {Array.from(
        { length: layers },
        (_, i) => <span key={i} className="deck-card-back" />,
      )}
    ...
    <div className="opp-hand">
      {Array.from(
        ...
        (_, i) => (
          <span
            key={i}
            className={"card-back" + (i === lift ? " lifted" : "")}
          />
        ),
      )}
  ...
  if (token.startsWith("hand:")) {
    el =
      document.querySelectorAll<HTMLElement>(
        ".opp-hand .card-back",
      )[Number(token.slice(5))] ?? null;
  } else {
    el = document.querySelector<HTMLElement>(
      `[data-cid="${CSS.escape(oppTokenCid(token))}"]`,
    );
  }
  if (!el) return null;
