...
export function Hero(props: {
  ...
  return (
    <div className={cls}>
      <div
        className="portrait"
        data-cid={side === "self" ? "selfHero" : "oppHero"}
        onClick={onClick}
      >
        {gems.length > 0 && (
          <div className="secret-gems">
            {gems.map((g) => (
              <span key={g.key} className="secret-gem">
                ...
                {g.card && (
                  <span
                    className={"card minion-preview secret-preview" +
                      cardColorClass(g.card)}
                    aria-hidden="true"
                  >
        ...
        {p.immune && (
          <div
            className="immune-badge"
            title="Immune — ignores all damage this turn"
          >
            🛡️
          </div>
        )}
        ...
        {p.weapon && (
          <div
            className="weapon-orb"
            title={`${p.weapon.name} — ${p.weapon.text ?? ""}`}
          >
