...
// TestDefaultWarlockDeckIsLegal: the curated Warlock fallback deck must be a legal
// Warlock deck (so vs-AI / fallback queuing with Warlock never builds an illegal
// game).
func TestDefaultWarlockDeckIsLegal(t *testing.T) {
	if err := ValidateDeck(DefaultDeckFor(ClassWarlock), ClassWarlock); err != nil {
		t.Fatalf("default Warlock deck must be legal: %v", err)
	}
}
...
// TestDefaultPaladinDeckIsLegal: the curated Paladin fallback deck must be a legal
// Paladin deck (so vs-AI / fallback queuing with Paladin never builds an illegal game).
func TestDefaultPaladinDeckIsLegal(t *testing.T) {
	if err := ValidateDeck(DefaultDeckFor(ClassPaladin), ClassPaladin); err != nil {
		t.Fatalf("default Paladin deck must be legal: %v", err)
	}
}
...
func TestDefaultRogueDeckIsLegal(t *testing.T) {
	if err := ValidateDeck(DefaultDeckFor(ClassRogue), ClassRogue); err != nil {
		t.Fatalf("default Rogue deck must be legal: %v", err)
	}
}
